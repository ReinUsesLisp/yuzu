...
void ConfigureGraphics::setConfiguration() {
    const bool powered_on{Core::System::GetInstance().IsPoweredOn()};

    ui->api->setEnabled(!powered_on);
    ui->api->setCurrentIndex(static_cast<int>(Settings::values.renderer_backend));
    ui->resolution_factor_combobox->setCurrentIndex(
        static_cast<int>(FromResolutionFactor(Settings::values.resolution_factor)));
    ui->toggle_frame_limit->setChecked(Settings::values.use_frame_limit);
    ui->frame_limit->setValue(Settings::values.frame_limit);
    ui->use_disk_shader_cache->setEnabled(!powered_on);
    ui->use_disk_shader_cache->setChecked(Settings::values.use_disk_shader_cache);
    ui->use_accurate_gpu_emulation->setChecked(Settings::values.use_accurate_gpu_emulation);
    ui->use_asynchronous_gpu_emulation->setEnabled(!powered_on);
    ui->use_asynchronous_gpu_emulation->setChecked(Settings::values.use_asynchronous_gpu_emulation);
    ui->enable_graphics_debugging->setEnabled(!powered_on);
...
void ConfigureGraphics::RetrieveVulkanDevices() {
    ...
    for (const auto physical_device : physical_devices) {
        ...
        vulkan_devices.push_back(properties.deviceName);
    }
#endif
}
