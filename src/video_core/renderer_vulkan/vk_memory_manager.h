...
public:
    ...
    /// Returns the writeable memory map. Do not call when the commit is not mappable.
    u8* GetData() const {
...
private:
    ...
    u8* data{}; ///< Pointer to the host mapped memory, nullptr when the commit is not mappable.
};
