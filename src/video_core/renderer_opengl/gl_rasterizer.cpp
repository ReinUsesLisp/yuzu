...
void RasterizerOpenGL::ConfigureFramebuffers(OpenGLState& current_state) {
    ...
    for (std::size_t index = 0; index < Maxwell::NumRenderTargets; ++index) {
        ...
        if (color_surface) {
            // Assume that a surface will be written to if it is used as a framebuffer, even if the
            // shader doesn't actually write to it.
            texture_cache.MarkColorBufferInUse(index);
            // Enable sRGB only for supported formats. Workaround for and issue in Nvidia drivers
