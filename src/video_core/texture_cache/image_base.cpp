...
ImageBase::ImageBase(const ImageInfo& info_, GPUVAddr gpu_addr_, VAddr cpu_addr_)
    : info{info_}, guest_size_in_bytes{CalculateGuestSizeInBytes(info)},
      host_size_in_bytes{CalculateHostSizeInBytes(info)}, gpu_addr{gpu_addr_}, cpu_addr{cpu_addr_},
      cpu_addr_end{cpu_addr + guest_size_in_bytes},
      layer_stride(info.type != ImageType::Linear ? CalculateLayerStride(info) : 0),
...
std::optional<SubresourceBase> ImageBase::FindSubresource(GPUVAddr other_gpu_addr) const noexcept {
    ASSERT(other_gpu_addr - gpu_addr < guest_size_in_bytes);

    const u32 diff = static_cast<u32>(other_gpu_addr - gpu_addr);
    const u32 layer = diff / layer_stride;
